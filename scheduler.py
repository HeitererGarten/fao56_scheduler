...
def generate_schedule(
    ...
    if not os.path.exists(weather_abs_path) or not os.path.exists(soil_abs_path):
        print("Weather or soil data file missing. Attempting to generate them using profile_prep...")
        try:
            profile_prep()
            # Re-check if files exist after running profile_prep
            if not os.path.exists(weather_abs_path):
                raise FileNotFoundError(f"Weather file still not found at expected location after running profile_prep: {weather_abs_path}")
            if not os.path.exists(soil_abs_path):
                raise FileNotFoundError(f"Soil data file still not found at expected location after running profile_prep: {soil_abs_path}")
            print("Successfully generated missing files.")
        except Exception as e:
            # Catching a broad exception here because profile_prep might raise various errors
            # and we also want to catch issues if the files are still not found.
            raise RuntimeError(f"Error during profile_prep execution or files still missing after attempt: {e}")
...
if __name__ == '__main__':
    ...
    print("Starting optimization and schedule generation process...")
    try:
        ...
        print(f"ERROR: A required data file was not found. Please ensure the 'db' directory and its contents exist relative to the script.")
        print(f"Details: {e}")
    except Exception as e:
